...
for regions in ['BB','BE','EE']:
     for i in range(1, hBase_mee_mr.GetNbinsX()+1):# for each mass bin
        ...
        c.SaveAs(str('~/public_html/Res_scale_15/fit_results/'+var_type+'_'+regions+'/'+hist_res.GetName()+'.png'))
        c.SaveAs(str('~/public_html/Res_scale_15/fit_results/'+var_type+'_'+regions+'/'+hist_res.GetName()+'.pdf'))
