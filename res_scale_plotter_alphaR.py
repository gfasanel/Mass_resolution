...
for regions in Regions:
    ...
    if 'alphaR' in variable_type:
        res_graph[regions].GetYaxis().SetTitle('alphaR')
    ...
    if "alphaR" in variable_type:
       ...
       x2=5250
       func=ROOT.TF1("func","[0] + [2]*(x-[1])*(x-[1])",x1,x2)
       func.SetParameter(0,0.025)
       func.SetParameter(1,1900)
       func.SetParLimits(1,1500,2400)
       func.SetParameter(2,-0.000000016) #for BE concavity is negative, it is positive for BB but slightly: 0.000000012

       res_graph[regions].Fit("func","R")
       label_fit =ROOT.TLatex()
       label_fit.SetNDC()
       label_fit.SetTextSize(0.04)
       label_fit.DrawLatex(0.3,0.8,"[0] + [2]*(x-[1])*(x-[1])")

       label_fit.DrawLatex(0.3,0.65,"[0]=%.2e "%(func.GetParameter(0)))
       label_fit.DrawLatex(0.3,0.6,"[1]=%.1lf "%(func.GetParameter(1)))
       label_fit.DrawLatex(0.3,0.55,"[2]=%.2e "%(func.GetParameter(2)))
       label_fit.Draw()
